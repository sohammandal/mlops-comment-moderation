...
class CheckpointManager:
    ...
    def save_checkpoint(
        self, total_batches: int, completed_batches: List[int], results: Dict[str, str]
    ) -> None:
...
def process_dataframe(
    df: pd.DataFrame, num_processes: Optional[int] = None, batch_size: int = 25
) -> List[str]:
    """
    ...
    for i in range(0, total_comments, batch_size):
        batch_texts = [
            (j, comments[j]) for j in range(i, min(i + batch_size, total_comments))
        ]
    ...
    if partial_results:
        ...
        logger.info(
            f"Resuming from checkpoint: {len(completed_batch_indices)} batches completed"
        )
    ...
    logger.info(
        f"Processing {total_comments:,} comments in {len(pending_batches)} batches "
    ...
    with ProcessPoolExecutor(
        ...
        for future in as_completed(future_to_batch_idx):
            ...
            processed_comments = min(
                completed_batches_count * batch_size, total_comments
            )

            # Save checkpoint periodically
            if completed_batches_count % 10 == 0:
                checkpoint_manager.save_checkpoint(
                    len(batches), list(completed_batch_indices), partial_results
                )
            ...
            if completed_batches_count % max(1, len(pending_batches) // 20) == 0:
                ...
                avg_speed = (
                    (
                        processed_comments
                        - len(checkpoint_manager.get_completed_batches()) * batch_size
                    )
                    / elapsed
                    if elapsed > 0
                    else 0
                ...
                logger.info(
                    f"Progress: {processed_comments:,}/{total_comments:,} comments "
...
def process_file(
    ...
    if debug:
        ...
        df = df.head(100)
        logger.info(
            f"Debug mode: Processing first 100 rows (out of {original_len} total)"
        )
    ...
    augmented_texts = process_dataframe(
        df, num_processes=num_processes, batch_size=batch_size
    )
...
Examples:
  ...
  %(prog)s --input large_file.csv --processes 96
    ...
    parser.add_argument(
        "--output", "-o", help="Output CSV file path (auto-generated if not specified)"
    )
    parser.add_argument(
        "--processes",
        "-p",
    ...
    try:
        df, output_path = process_file(
            args.input, args.output, args.processes, args.debug
        )
        ...
        new_augmentations = sum(
            1
            for orig, aug in zip(original_comments, augmented_comments)
            if aug is not None and orig != aug
        )

        # A more accurate cache hit calculation
        cache_hits = (
            sum(1 for aug in augmented_comments if aug is not None) - new_augmentations
        )
        ...
        if elapsed_total > 0:
            logger.info(
                f"Processing rate: {total_comments / elapsed_total:.1f} comments/second"
            )
